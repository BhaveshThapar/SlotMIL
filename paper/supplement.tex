...
The pre-registered denominator for \pns{} is an in-plane template fit on
validation to \emph{the arm's own attention}. Fit to the supervised probe's
output that identical rule scores $0.8358$; fit to a MIL arm's attention its
median over the $40$ scored dumps on \condA{} is $0.4208$, and it sits below
$0.5$ for $24$ of them. Since $s = (A - A_t)/(1 - A_t)$, a sub-chance $A_t$
