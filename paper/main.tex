...
% One block per author. The `\\` must sit inside each block: a single trailing
% run of lines after `\and` attaches to the *second* column only, which silently
% files the first author's affiliation and e-mail under the second author.
\author{
  Bhavesh Thapar\\
...
measures. Across ten MIL architectures on LIDC-IDRI, five seeds each, flat (3D)
instance AUC sits within $0.02$ of within-slice AUC for seven of eight scored
arms, no arm exceeds a model-free centre prior on the axial axis (the best
statistically ties it), the $95$th
percentile over $30$ untrained initialisations is $0.7666$ --- so $0.5$ is not
the floor --- and a content-free \maskref{} built from lesion masks alone
reaches $\maskrefSep$ (separable member; in-plane $\maskrefInplane$), above
...
is reported as three-dimensional localisation. That attention may be doing less
work than assumed is already visible at the classification level, where mean
pooling matches or beats attention MIL across seven 3D neuroimaging
tasks~\citep{harvey2026benchmark}; we ask the corresponding question of the
localisation number.
...
\emph{lesion masks}, the \maskref{} of \S3 --- scores above every trained arm,
and an untrained network reaches $0.7666$ at the $95$th percentile. The apparent skill sits between a floor that is nowhere near
...
Harvey-style \armNG{}~\citep{harvey2026normal} adds a KL term pulling the attention's
...
consistency checks; \condD{} runs the content-free template family only,
carries H9 alone, and receives no verdict table. Where this paper says ``all
three conditions'' it means the three LIDC conditions (the companion states the
declared-versus-reported diff in full). Which condition leads was ruled on
...
Gaussian and \armNG{}~\citep{harvey2026normal}. Five seeds each.
...
\paragraph{Two templates, two names.} Two distinct objects in this paper are
each an in-plane template fit on validation, and a reader who conflates them
will conclude that our denominator is the thing beating the trained arms. The
\emph{\maskref{}} is fit to validation \emph{lesion masks}: it reads no image
and no attention, it sets the content-free ceiling ($\maskrefInplane$
in-plane, $\maskrefSep$ separable), and it appears in the figures as the
starred references. The \emph{\attden{}} is the same fitting rule applied to
\emph{the arm's own attention}: it is the pre-registered reference $A_t$
below, and its median over the scored dumps is $\attdenMedian$. The first is
an oracle no arm reaches; the second adapts to the arm being scored.
...
tables. The reference $A_t$ is the \attden{} --- tighter than an unfit average
mask~\citep{arun2021saliency} and tighter than a fixed $\sigma{=}1$
Gaussian~\citep{harvey2026normal}, so it shrinks the denominator and lowers
every reported skill, which is the honest direction.

One property of that reference is worth stating in advance, because it is the
mechanism and not a defect. Because the \attden{} is fit to the arm's own
attention, what it scores depends on whose attention it was fit to. Fit to the
supervised probe's output the identical rule scores $\probeDen$; fit to a MIL
arm's attention its median over the scored dumps is $\attdenMedian$, below
chance, because their attention carries no in-plane structure to fit. A
sub-chance denominator inflates $1 - A_t$ above $1$ and so shrinks the
...
are \maskref{}s: they read no image features at all and are computed from
lesion masks and patch geometry. The best trained arm --- the \armSlot{}, at
$0.8247$ --- sits below the separable \maskref{} and barely above an untrained
network's $95$th percentile.
...
slice AUC of $\centrePriorSlice$; the best is \armNG{} at $0.6318$. That
margin is $0.0109$ and deserves its interval stated rather than demanded: the
paired per-bag difference (prior $-$ arm), patient-clustered over the arm's
five seeds, is $+0.0109$ $[-0.0078, +0.0300]$ for \armNG{} ---
statistically indistinguishable --- while every other arm's interval sits
entirely on the prior's side, and on both consistency conditions the interval
excludes zero even for \armNG{}. The claim, stated at the strength the data
supports: no trained arm exceeds a prior with no parameters and no fitting on
the axis, and the best arm, on the family condition alone, ties it.
...
poolings. Chance is not the floor for this protocol: a reported $0.79$--$0.82$
is inside or barely above the untrained range, and our own best arm clears the
percentile by under $0.06$ (\S\ref{sec:slot}).
...
declared \attden{} sits below chance for $24$ of $40$ scored dumps
...
\subsection{The best trained arm, read honestly}
\label{sec:slot}

The strongest trained result in this paper belongs to the \armSlot{} --- slot
attention with a diversity term, the project's founding hypothesis --- and it
is the cleanest single illustration of the critique. Four facts about one
arm, each from a different artefact. Its flat instance AUC of $0.8247$ is the
best of the eight, above the untrained $95$th percentile ($0.7666$) and below
the separable \maskref{} ($\maskrefSep$). Its \attden{} suffers the largest
in-lung collapse of any arm, $-0.300$ against a next-largest of $+0.005$
(Figure~\ref{fig:lung}). H5's one over-bar seed ($0.3432$,
\S\ref{sec:h4h5}) is a slot seed, and the arm holds H5's maximum
($+0.1990$). And Table~\ref{tab:armlung} applies Recommendation~1 to the arms
themselves rather than to their templates: restricted to the lung, the
\armSlot{}'s own attention falls from $0.8247$ to $0.5560$ --- a loss of
$0.2688$, far the largest of the eight, while the arms with no headroom drift
toward chance from below.

An ``interpretable'' pooling bottleneck looked best precisely because it had
learned where lungs are. The arm our founding hypothesis picked is not an
exception to the critique; it is its sharpest example.

\begin{table}[t]
\centering
\input{tables/arm_in_lung}
\caption{Raw and in-lung instance AUC of each arm's \emph{own} frozen-slot
attention (mean over five seeds, \condA{}; patient-clustered bootstrap per
seed). Recommendation~1 applied to this paper's own arms: the scored quantity
here is the attention itself, not the \attden{} of Figure~\ref{fig:lung}.
Raw values are verified against the confirmatory
\texttt{template\_family.json} at generation time.}
\label{tab:armlung}
\end{table}
...
($A = 0.9558$ against its own \attden{} at $\probeDen$). The content-free set:
chance $0.0$, roll permutation $+0.0105$, entropy-matched random $-0.0001$,
the \texttt{fitted\_template} member (the \attden{} itself) $0.0$ by
construction. The estimands discriminate.

\paragraph{Scope, stated up front.} The gate is per condition, and it does not
pass everywhere: \condA{} $0.7305$, \condB{} $0.7036$, \condC{}
$\mathbf{0.4772}$ against the $0.50$ bar --- the three LIDC conditions;
\condD{} is out of the gate's scope by declaration. On \condC{} the probe's own
\attden{} is the strongest of the three ($0.8568$) while its AUC is the weakest
...
does not reach the model-free centre prior at $\centrePriorSlice$, so H2 holds
for it too --- as a statistical tie, the only arm to manage even that.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{figures/fig3_in_lung.pdf}
\caption{The \attden{}, scored on the whole bag and restricted to a lung
segmentation. Note the scored quantity is the template fit to each arm's
attention, not the arm's own attention: for most arms it is already at chance
unrestricted, so only the arms whose template carried in-plane structure have
anything to lose. The arms' own attention under the identical restriction is
Table~\ref{tab:armlung}.}
\label{fig:lung}
\end{figure}
...
inside it the pre-registered \attden{} scores $0.4884$ against a bar of $0.65$
--- a bar set, in the amendment of 2026-08-15, against the unrestricted
\attden{}'s $0.786$ on the discovery split --- with the position-stratified
variant at $0.4936$ beside it and all three LIDC conditions agreeing
($0.4884 / 0.5080 / 0.4940$). No arm's \attden{} beats chance inside the lung.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig4_dataset_contrast.pdf}
...
finding, and required MosMed's \attden{} AUC to sit below LIDC's with the
...
measurement. The references that set the ceiling --- the in-plane \maskref{} at
$\maskrefInplane$, the separable at $\maskrefSep$, the centre prior at
...
against the \attden{} --- an in-plane template fit on validation to the
method's own attention --- and report the per-condition supervised-probe gate
beside it. Where the gate fails, fall back to recommendation~1.
\end{enumerate}
